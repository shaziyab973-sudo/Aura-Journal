...
try {
  ...
  console.warn(
    "Could not read firebase-applet-config.json on startup:",
    err
  );
}
  ...
  for (const model of MODEL_FALLBACK_LADDER) {
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        console.warn(
          `Model ${model} attempt ${attempt}/2 failed: ${errorMessage}`
        );
        ...
        if (attempt < 2) {
          if (temporaryError) {
            console.log(
              `Temporary Gemini error. Retrying ${model} in 2 seconds...`
            );
    ...
    console.warn(
      `Moving to next Gemini model after ${model} failed.`
    );
  }
...
// =============================================================
function computeHeuristicSentiment(
  text: string,
  mood: string
) {
  ...
  if (
    ["grateful", "inspired", "peaceful", "joyful"].includes(
      moodLower
    )
  ) {
    baseScore = 0.55;
  ...
  if (
    ["stressed", "overwhelmed", "anxious", "down"].includes(
      moodLower
    )
  ) {
    baseScore = -0.45;
  ...
  const overallScore = Math.max(
    ...
    Math.min(
      0.9,
      parseFloat((baseScore + wordDiff).toFixed(2))
    )
  );
  ...
  if (overallScore >= 0.4) {
    primaryEmotion = "Grateful & Optimistic";
  } else if (overallScore >= 0.1) {
    primaryEmotion = "Thoughtful & Calm";
  } else if (overallScore >= -0.3) {
    primaryEmotion = "Contemplative";
  } else {
    primaryEmotion = "Vulnerable & Processing";
  }
  ...
  return {
    ...
    keyDrivers: [
      mood
        ? `Current emotional state identified as "${mood}"`
        : "Daily reflection themes",
      posCount > 0
        ? "Noted positive reflections & gratitude"
        : "Navigating current life commitments",
    ],
    cognitiveInsight:
      "A thoughtful introspective reflection highlighting your commitment to self-awareness and mindful processing.",
    growthOpportunity:
      "Take a quiet moment to honor how much you are navigating today, and grant yourself permission to proceed step by step.",
    analyzedAt: Date.now(),
  };
}

// =============================================================
// HEURISTIC TASK EXTRACTION FALLBACK
// =============================================================
function extractHeuristicTasks(
  text: string,
  sessionTitle: string
) {
  ...
  const taskRegex =
    /(?:need to|have to|plan to|will|should|must|remember to|todo|task|action)\s+([^.!?\n]{5,75})/gi;
    ...
    while ((match = taskRegex.exec(trimmed)) !== null) {
      if (
        ...
        tasks.push({
          ...
          category:
            /work|job|project|deadline|email/i.test(title)
              ? "Career"
              : /walk|sleep|rest|meditate|health|water/i.test(
                  title
                )
              ? "Wellness"
              : "Personal",
  ...
  if (tasks.length === 0) {
    tasks.push({
      title:
        "Dedicate 10 minutes to quiet pause and hydration",
      priority: "low",
      category: "Wellness",
    ...
    tasks.push({
      title:
        "Review insights and feelings logged in today's entry",
      priority: "medium",
      category: "Personal",
      context:
        "Revisit your reflections to ground your evening thoughts",
...
// =============================================================
// HEURISTIC JOURNAL REFLECTION FALLBACK
// =============================================================
...
${
  mood
    ? `Noting that you are feeling **${mood}**, it is natural for moments like this to invite deeper contemplation.`
    : ""
}
...
// =============================================================
// HEURISTIC ACTION FALLBACK
// =============================================================
function generateHeuristicAction(
  action: string,
  entriesText: string,
  mood: string,
  topic: string
...
1. Tone:
Warm, supportive, contemplative, non-judgmental, clear, concise, and encouraging.

2. Empathy:
Acknowledge the user's emotional state gently with active listening before asking thoughtful questions.
...
- Ask 1 or at most 2 gentle, open-ended reflection questions.
- Invite deeper self-awareness.
- Do not overwhelm the user with long lectures or generic motivational clichés.

5. Formatting:
Use markdown thoughtfully with paragraphs, bullet points, and subtle bolding to make reflections calming and easy to read.
`;
...
app.get("/api/health", (_req, res) => {
  res.json({
    status: "ok",
    service:
      "AI Personal Journal & Reflection Assistant",
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Missing journal entry or messages array.",
      });
    }
    ...
    if (contents.length === 0) {
      return res.status(400).json({
        error:
          "No valid journal content provided.",
      });
    }

    const moodContext = mood
      ? `The user flagged their current mood as: "${mood}". Reflect with appropriate emotional resonance.`
      : "";

    const systemInstruction =
      `${JOURNAL_ASSISTANT_SYSTEM_PROMPT}\n${moodContext}`;
    ...
    res.status(500).json({
      error:
        error?.message ||
        "Failed to generate reflection response from Gemini.",
    });
  }
});
  ...
  try {
    ...
    if (action === "summarize") {
      if (
        ...
        return res.status(400).json({
          error:
            "Journal session content is required to summarize.",
        });
      }
...
Provide 3 to 4 actionable, encouraging, and realistic ideas.

Keep the tone grounded, kind, and inspiring.
`;
...
`;
    ...
    try {
      ...
      console.warn(
        `AI models temporarily unavailable for action ${action}, using resilient fallback:`,
        aiErr?.message
      );

      text = generateHeuristicAction(
        action,
        entriesText,
        mood,
        topic
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Reflection text is required for sentiment analytics.",
        });
      }
...
You MUST return ONLY a valid raw JSON object matching this exact schema:
...
Rules:
- overallScore must be between -1.0 and 1.0.
...
`;
      ...
      try {
        ...
        console.warn(
          "AI models temporarily unavailable for sentiment, activating resilient local heuristic:",
          aiErr?.message
        );

        const heuristic =
          computeHeuristicSentiment(
            text,
            mood
          );
      ...
      try {
        ...
        console.warn(
          "Direct JSON parse failed, extracting first JSON block:",
          parseError
        );
      ...
      res.json({
        sentiment: {
          ...
          cognitiveInsight:
            parsed.cognitiveInsight ||
            "A reflective mindset showing thoughtful emotional self-awareness.",

          growthOpportunity:
            parsed.growthOpportunity ||
            "Take a few minutes to acknowledge your resilience and progress today.",
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const {
        text = "",
        sessionTitle = "Journal Reflection",
      } = body;
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Text is required to extract action items.",
        });
      }
...
`;
      ...
      try {
        ...
        console.warn(
          "AI models temporarily unavailable for task extraction, using resilient fallback:",
          aiErr?.message
        );

        return res.json({
          tasks: extractHeuristicTasks(
            text,
            sessionTitle
          ),
          modelUsed: "heuristic-fallback",
      ...
      res.json({
        tasks: rawTasks.map((task: any) => ({
          ...
          context:
            task.context ||
            `Extracted from session: ${sessionTitle}`,
      ...
      console.error(
        "Error in /api/gemini/extract-tasks:",
        error
      );
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Valid latitude and longitude numbers are required.",
        });
      }

      // Fuzz coordinates to approximately 1.1 km
      const fuzzedLat =
      ...
      if (mapsApiKey) {
        try {
          const url =
            `https://maps.googleapis.com/maps/api/geocode/json?latlng=${fuzzedLat},${fuzzedLng}&key=${mapsApiKey}`;
          ...
          console.warn(
            "Google Maps reverse geocoding request failed, falling back to local resolver:",
            mapError
          );
        }
      }
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "Places search failed, using curated suggestions:",
            error
          );
        }
      }
      ...
      const curated = [
        {
          ...
          neighborhood: "Personal Studio",
          city: "Private Space",
          isFuzzed: true,
        },

        {
          placeName: "Quiet Coffee House",
...

        {
          ...
          neighborhood: "Reading Room",
          city: "Study Alcove",
          isFuzzed: true,
        },

        {
          placeName: "Mountain Trailhead",
          neighborhood: "Alpine Meadow",
          city: "Highland Retreat",
          isFuzzed: true,
        },

        {
          placeName: "Ocean Promenade",
          neighborhood: "Coastline Walk",
          city: "Seaside Haven",
          isFuzzed: true,
        },
      ].filter(
...
async function startServer() {
  ...
  app.listen(
    ...
    () => {
      console.log(
        `AI Journal server listening on http://0.0.0.0:${PORT}`
      );
    }
  );
}
